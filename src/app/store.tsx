...
function rewardImage(reward: RewardItem) {
  if (reward.code === "CONVENIENCE_3000")
    return require("@/assets/images/rewards/cu-3000.jpg");
  if (reward.code === "LOCAL_CAFE_10")
    return require("@/assets/images/rewards/starbucks.png");
  return null;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <View style={styles.balanceCopy}>
            ...
            <Text style={[styles.balanceValue, { color: colors.text }]}>
              ...
              <Text style={[styles.balanceUnit, { color: colors.green }]}>
                P
              </Text>
            </Text>
          </View>
          <View
            style={[styles.balanceIcon, { backgroundColor: colors.greenSoft }]}
          >
        ...
        >
          <View
            style={[styles.fundingIcon, { backgroundColor: colors.greenSoft }]}
          >
            <Ionicons name="business-outline" size={22} color={colors.green} />
          </View>
          <View style={styles.fundingCopy}>
            <Text style={[styles.fundingTitle, { color: colors.text }]}>
              혜택은 어떻게 만들어지나요?
            </Text>
            <Text style={[styles.fundingText, { color: colors.muted }]}>
              지자체 운영비와 기업 CSR 보상 예산, 동네 가게의 할인 혜택으로
              포인트 사용처를 운영해요.
            </Text>
          </View>
        </View>
    ...
    <View style={styles.rewardFrame}>
      {limited ? (
        <Animated.View
          ...
          style={[
            styles.limitedGlow,
            { borderColor: colors.green, shadowColor: colors.green },
            glowStyle,
          ]}
        />
      ) : null}
      <Pressable
        ...
        style={({ pressed }) => [
          styles.rewardCard,
          {
            ...
            borderColor: limited ? colors.green : colors.border,
          },
      ...
      >
        {image ? (
          ...
          <View
            style={[styles.rewardIcon, { backgroundColor: visual.tint + "22" }]}
          >
            <Ionicons
              name={limited ? "ribbon" : visual.icon}
              size={limited ? 29 : 26}
              color={visual.tint}
            />
            {limited ? (
              <View
                style={[styles.sparkleDot, { backgroundColor: colors.green }]}
              />
            ) : null}
          </View>
        )}
        <View style={styles.rewardCopy}>
          <View style={styles.rewardTop}>
            <View style={styles.rewardLabelRow}>
              <Text style={[styles.rewardType, { color: colors.green }]}>
              ...
              {limited ? (
                ...
                >
                  <Ionicons name="sparkles" size={9} color="#17310b" />
            ...
            <Text
              style={[
                styles.rewardStock,
                { color: limited ? colors.orange : colors.muted },
              ]}
            >
          ...
          <Text
            numberOfLines={2}
            style={[styles.rewardDescription, { color: colors.muted }]}
          >
            {reward.description}
          </Text>
          <View style={styles.rewardBottom}>
            <Text style={[styles.sponsor, { color: colors.muted }]}>
              {reward.sponsorName}
            </Text>
            <Text
              style={[
                ...
                {
                  color:
                    balance >= reward.pointPrice ? colors.text : colors.orange,
                },
              ]}
            >
    ...
    >
      <View style={styles.modalBackdrop}>
        ...
        >
          ...
          <View style={styles.sheetHeader}>
            {image ? (
              ...
              <View
                style={[
                  styles.sheetIcon,
                  { backgroundColor: visual.tint + "22" },
                ]}
              >
                <Ionicons
                  name={
                    reward.code === "COMMUNITY_BADGE" ? "ribbon" : visual.icon
                  }
                  size={30}
                  color={visual.tint}
                />
          ...
          <Text style={[styles.sheetType, { color: colors.green }]}>
            {typeLabel(reward.type)} · {reward.sponsorName}
          </Text>
          <Text style={[styles.sheetTitle, { color: colors.text }]}>
            {reward.title}
          </Text>
          <Text style={[styles.sheetDescription, { color: colors.muted }]}>
            {reward.description}
          </Text>
          <View
            style={[styles.termCard, { backgroundColor: colors.surfaceRaised }]}
          >
            <Ionicons
              ...
              color={colors.green}
            />
            <Text style={[styles.termText, { color: colors.muted }]}>
              {reward.terms}
              {"\n"}교환하면 내 혜택에 저장되고, 이웃에게 보낼 선물 코드도 함께
              발급돼요.
            </Text>
          </View>
          ...
          {error ? (
            <Text
              ...
              style={[styles.sheetError, { color: colors.orange }]}
            >
              {error}
            </Text>
          ) : null}
          <Pressable
            ...
            style={[
              ...
              (loading || insufficient) && styles.disabled,
            ]}
          >
            <Text
              style={[
                styles.purchaseButtonText,
                { color: insufficient ? colors.muted : "#17310b" },
              ]}
            >
              {loading
                ? "교환하고 있어요"
                : insufficient
                  ? "포인트가 부족해요"
                  : `${reward.pointPrice.toLocaleString()} P로 교환하기`}
            </Text>
          </Pressable>
          <Text style={[styles.demoNotice, { color: colors.muted }]}>
            이 혜택은 실제 금전적 효력이 부족합니다.
          </Text>
        </SafeAreaView>
      </View>
      <StatusBar style={mode === "dark" ? "light" : "dark"} />
...
function ReceiptModal({
  ...
  const shareCode = async () => {
    await Share.share({
      message: `동네모아에서 ${receipt.title} 혜택을 선물했어요.\n선물 코드: ${receipt.redemptionCode}\n※ 발표용 데모 혜택이에요.`,
    });
  };
  return (
    <Modal
    ...
    >
      <View style={styles.receiptBackdrop}>
        ...
        >
          <View
            style={[styles.successIcon, { backgroundColor: colors.greenSoft }]}
          >
            <Ionicons name="gift" size={32} color={colors.green} />
          </View>
          <Text style={[styles.receiptTitle, { color: colors.text }]}>
            내 혜택에 보관했어요
          </Text>
          <Text style={[styles.receiptName, { color: colors.muted }]}>
            {receipt.title}
          </Text>
          <View
            style={[
              styles.inventoryNotice,
              { backgroundColor: colors.greenSoft },
            ]}
          >
            <Ionicons name="checkmark-circle" size={19} color={colors.green} />
            <Text style={[styles.inventoryNoticeText, { color: colors.text }]}>
              마이페이지 보관함에 저장 완료
            </Text>
          </View>
          <View
            style={[styles.codeBox, { backgroundColor: colors.surfaceRaised }]}
          >
            ...
            <Text selectable style={[styles.code, { color: colors.text }]}>
              {receipt.redemptionCode}
            </Text>
          </View>
          <Text style={[styles.receiptNotice, { color: colors.muted }]}>
            직접 사용해도 되고, 선물 코드를 공유해 이웃에게 보낼 수도 있어요.
          </Text>
          <Pressable
            onPress={() => {
              void shareCode();
            }}
            style={[styles.receiptButton, { backgroundColor: colors.green }]}
          >
            <Ionicons name="share-social-outline" size={18} color="#17310b" />
            <Text style={styles.receiptButtonText}>선물 코드 보내기</Text>
          </Pressable>
          <Pressable
            onPress={onGoInventory}
            style={[styles.inventoryButton, { borderColor: colors.border }]}
          >
            <Text style={[styles.inventoryButtonText, { color: colors.text }]}>
              내 혜택에서 확인
            </Text>
          </Pressable>
        </View>
