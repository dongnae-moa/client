...
import { applyProfileDecoration, getMyRewards, getPointBalance } from "../api/rewards";
...
export default function MyScreen() {
  ...
  const loadBenefits = useCallback(async () => {
    try {
      const [items, profile, point] = await Promise.all([getMyRewards(), refreshProfile(), getPointBalance()]);
      setBenefits(items);
      updateUser({ ...profile, point: point.balance });
      setBenefitError(null);
    } catch (requestError) {
      setBenefitError(requestError instanceof ApiError ? requestError.message : "내 혜택을 불러오지 못했어요.");
    }
  ...
  const applyDecoration = async (benefit: RewardRedemption) => {
    ...
    try {
      ...
      setBenefitError(requestError instanceof ApiError ? requestError.message : "프로필 장식을 적용하지 못했어요.");
    } finally {
      setApplyingId(null);
    }
  };
  ...
  const shareBenefit = async (benefit: RewardRedemption) => {
    await Share.share({ message: `동네모아에서 ${benefit.title} 혜택을 선물했어요.\n선물 코드: ${benefit.redemptionCode}\n※ 발표용 데모 혜택이에요.` });
  };

  const stats = [
    { icon: "star-outline" as const, value: (user?.point ?? 0).toLocaleString(), label: "보유 포인트", color: colors.orange },
    { icon: "checkmark-circle-outline" as const, value: "24", label: "미션 완료", color: colors.green },
    { icon: "people-outline" as const, value: "12", label: "참여 일수", color: colors.purple },
    { icon: "flame-outline" as const, value: "5", label: "연속 참여", color: colors.orange },
  ];
  ...
  return (
    <ScreenSurface>
      ...
      <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: colors.text }]}>내 혜택</Text><Pressable onPress={() => router.navigate("/map")}><Text style={[styles.more, { color: colors.green }]}>상점 보기 ›</Text></Pressable></View>
      <SurfaceCard>
        {benefits.length === 0 ? <View style={styles.emptyBenefits}><Ionicons name="gift-outline" size={24} color={colors.green} /><Text style={[styles.emptyTitle, { color: colors.text }]}>아직 보관한 혜택이 없어요</Text><Text style={[styles.emptyText, { color: colors.muted }]}>미션 포인트로 첫 혜택을 교환해보세요.</Text></View> : benefits.slice(0, 4).map((benefit, index) => (
          <View key={benefit.id} style={[styles.benefitRow, index < Math.min(benefits.length, 4) - 1 && { borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth }]}>
            <View style={[styles.benefitIcon, { backgroundColor: benefit.type === "PROFILE_DECORATION" ? colors.greenSoft : colors.surfaceRaised }]}><Ionicons name={benefit.type === "PROFILE_DECORATION" ? "leaf-outline" : benefit.type === "GIFTICON" ? "gift-outline" : "storefront-outline"} size={20} color={benefit.type === "PROFILE_DECORATION" ? colors.green : colors.purple} /></View>
            <View style={styles.benefitCopy}><Text style={[styles.benefitTitle, { color: colors.text }]}>{benefit.title}</Text><Text numberOfLines={1} style={[styles.benefitCode, { color: colors.muted }]}>선물 코드 · {benefit.redemptionCode}</Text></View>
            <View style={styles.benefitActions}>
              {benefit.type === "PROFILE_DECORATION" ? <Pressable disabled={applyingId === benefit.id || user?.profileDecorationKey === benefit.decorationKey} onPress={() => { void applyDecoration(benefit); }} style={[styles.applyButton, { backgroundColor: user?.profileDecorationKey === benefit.decorationKey ? colors.surfaceRaised : colors.green }]}><Text style={[styles.applyText, { color: user?.profileDecorationKey === benefit.decorationKey ? colors.muted : "#17310b" }]}>{user?.profileDecorationKey === benefit.decorationKey ? "적용 중" : applyingId === benefit.id ? "적용 중" : "적용"}</Text></Pressable> : null}
              <Pressable accessibilityLabel={`${benefit.title} 선물 코드 보내기`} onPress={() => { void shareBenefit(benefit); }} style={[styles.shareButton, { backgroundColor: colors.surfaceRaised }]}><Ionicons name="paper-plane-outline" size={15} color={colors.purple} /></Pressable>
            </View>
          </View>
        ))}
        {benefitError ? <Text accessibilityRole="alert" style={[styles.benefitError, { color: colors.orange }]}>{benefitError}</Text> : null}
      </SurfaceCard>
...
const styles = StyleSheet.create({
  ...
  statLabel: { fontFamily: "WantedSansR", fontSize: 9, marginTop: 3, textAlign: "center" },
  ...
  benefitIcon: { alignItems: "center", borderRadius: 13, height: 42, justifyContent: "center", width: 42 },
  ...
  shareButton: { alignItems: "center", borderRadius: 999, height: 31, justifyContent: "center", width: 31 },
  benefitError: { fontFamily: "WantedSansR", fontSize: 9, lineHeight: 14, marginTop: 9 },
  badges: { flexDirection: "row", justifyContent: "space-between" },
  badge: { alignItems: "center", flex: 1 },
  badgeIcon: { alignItems: "center", borderRadius: 999, height: 54, justifyContent: "center", width: 54 },
