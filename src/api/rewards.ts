...
export async function getRewards(type?: RewardType) {
  ...
  try {
    return await apiRequest<RewardItem[]>(`/v1/rewards${query}`);
  } catch (error) {
...
export async function getPointBalance() {
  try {
    return await apiRequest<{ balance: number }>("/v1/users/me/points");
  } catch (error) {
    if (!shouldUseDemoRewards(error)) throw error;
...
export async function getMyRewards() {
  try {
    return await apiRequest<RewardRedemption[]>("/v1/users/me/rewards");
  } catch (error) {
    if (!shouldUseDemoRewards(error)) throw error;
